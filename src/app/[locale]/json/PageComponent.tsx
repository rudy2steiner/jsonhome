  ...
  return (
    <>
      <HeadInfo
        ...
        page={"/json"}
      />
    <Header locale={locale} page={"json"} indexLanguageText={indexLanguageText}/>
    <div className={"w-[90%] mx-auto"} id="left">
    </div>
    {
        languages.map((item) => {
          let hrefLang = item.code1
          if (locale == item.lang) {
            return  <p className="text-white" id={"lang-1"} >{hrefLang}</p>
          }
        })
      }
    <Script id="show-banner">
          {` // create the editor
            ...
            const lang = document.getElementById("lang-1").innerText
           console.log(lang)
            const options = {
                language: lang,
                mode: 'code',
